...
var cnEnvironment = function(cb) {
   ...
   del(config.rootBuild).then(paths => {
        ...
        // 执行项目打包
        runSequence(
           'htmlmin', 'cssmin', 'images', 'jsmin', 'libmin','revhtml'
        , function() {
...
gulp.task('cssmin', function() {
    ...
    return gulp
        ...
        //.pipe(rename({suffix: '.min'}))//添加.min
        .pipe(rev())//文件名加MD5后缀
        .pipe(gulp.dest(config.build.css))
        .pipe(rev.manifest('rev-css-manifest.json'))　//mainfest.json文件，里面是压缩文件的信息
...
gulp.task('jsmin', ['eslint'], function() {
    var jsmin = gulp
        ...
        .pipe(rev.manifest('rev-js-manifest.json'))　//mainfest.json文件，里面是压缩文件的信息
        .pipe(gulp.dest(config.build.rev))
        .pipe(reload({ stream: true }));
...
gulp.task('revhtml', function() {
    //html，针对css，js
    return gulp
    .src([config.build.rev+'*.json', config.build.buildHtml])
    .pipe(revCollector({
			 replaceReved: true
		}))
...
gulp.task('libmin', function() {
    ...
    return gulp
        .src(config.dev.lib)
        .pipe(concat('build.js'))//合并所有的公共js
        .pipe(plumber(onError))
